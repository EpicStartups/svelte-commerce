...
interface Data {
    ...
    deliveryDetails: any,
    streamed: {
        moreProductDetails: Promise<any>,
        productReviews: Promise<any>
    }
}
...
let seoProps = generateSeoProps({
	...
	ratingCount: 1,
	ratingValue: 5, // +data.product?.ratings + 1,
	...
	datePublished: "_", //`${data.product?.publishedAt || '_'}`,
...
<div class="min-h-screen lg:p-10 font-jost">
	<div class="md:container md:mx-auto flex flex-col gap-5">
		...
		<div class="mb-5 grid grid-cols-1 items-start gap-5 sm:mb-10 sm:gap-10 lg:grid-cols-5">
			...
			<div class="col-span-1 lg:col-span-2 px-3 sm:px-10 lg:px-0 flex flex-col gap-5">
				<div>
					...
					{#if data?.product?.brand  && data?.product?.brandName}
						<p>
							By

							<a href="/store/{data.product.brandName}" class="underline hover:text-zinc-800">
									...
									<li>
										<a
											...
											{#if mgp.material.price}
												<span
													><b>{currency(mgp.material.price, "RM")}</b
													></span>
											{/if}
										</a>
									</li>
											...
											<form
												...
												use:enhance="{() => {
													return async ({ result }) => {
														...
														data?.qty < 0
															? fireGTagEvent('remove_from_cart', data)
															: fireGTagEvent('add_to_cart', data)
		...
		<div class="px-3 sm:px-10 lg:px-0 flex flex-col gap-5 sm:gap-10">
			...
			<CategoryPoolButtons categoryPool="{[]}" />
