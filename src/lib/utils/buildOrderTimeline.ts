...
export const useBuildTimeline = (order: MedusaOrder) => {
	...
	// const { order_edits: edits, isLoading: isOrderEditsLoading } = useAdminOrderEdits({
	// 	order_id: orderId
	// })
	...
	for (const event of order.fulfillments) {
		...
		if (event.shipped_at) {
			events.push({
				id: event.id,
				time: new Date(event.shipped_at),
				type: 'shipped',
				items: event.items.map((item) => getFulfilmentItem(allItems, [], item)),
				noNotification: event.no_notification,
				orderId: order.id,
...
function findOriginalItemId(edits, originalId) {
	...
	edits = edits
		...
		.sort((a: any, b: any) => (new Date(a.confirmed_at) as any) - (new Date(b.confirmed_at) as any))
