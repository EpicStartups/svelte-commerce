...
import type { MedusaCart } from '$lib/services/medusa/types'
	...
	<section>
		...
		<div class="mt-3 text-sm">
			...
			<div class="mt-2 flex items-center justify-between font-medium">
				...
				<span>
					{#if cart.formattedAmount?.shipping?.raw / 100 < 1 || cart.shipping?.charge < 1}
						<span
							class="border border-accent-500 text-accent-500 uppercase px-1.5 py-0.5 rounded text-xs">
							Free
						</span>
					{:else}
						{cart.formattedAmount.shipping.value}
					{/if}
				</span>
			</div>
		</div>
