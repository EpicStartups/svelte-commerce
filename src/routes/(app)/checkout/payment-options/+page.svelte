...
import { stripePublicKey } from '$lib/config/index.js'
import type { MedusaPaymentSession } from '$lib/services/medusa/types.js'
import Stripe from '$lib/components/Stripe.svelte'
import ShippingOptions from '../_ShippingOptions.svelte'
...
let selectedPaymentMethod = { id: '', name: '', text: '', instructions: '', qrcode: '', img: '', value: '' }
let showPayWithBankTransfer = false

$: if (data.paymentSessions.cart.payment_sessions?.length === 1) {
...
function paymentMethodChanged(pm: MedusaPaymentSession) {
...
<div class="container mx-auto min-h-screen w-full max-w-6xl p-3 py-5 sm:p-10">
	...
	<div
		...
		<div class="w-full flex-1">
			...
			<h2 class="mb-5 mt-12">Payment Options</h2>

			{#if data.paymentSessions?.cart.payment_sessions.length}
				<div class="flex w-full flex-col gap-4" class:wiggle="{paymentDenied}">
					{#each data.paymentSessions?.cart.payment_sessions as pm}
						<label
							...
							<input
								bind:group="{selectedPaymentMethod.id}"
								type="radio"
								value="{pm.id}"
								name="group"
						...
						{#if pm.provider_id === 'stripe'}
							<Stripe 
								address={data.address}
								cart={data.medusaCart}
								cartId={data.medusaCart.id}
								clientSecret={pm.data.client_secret}
		...
		<div class="w-full md:w-80 md:shrink-0 md:grow-0">
			...
			{#if data.address}
				<div class="mb-5">
					...
					<p>
						{data.address.first_name || '_'}
						{data.address.last_name || '_'}
